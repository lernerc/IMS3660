...
if(isset($_COOKIE["username"])){
   ...
   echo "<form action=\"deletecontains.php\" method=post>";/*itemContains.php\" method=post>";*/
...
*/
   echo "<h3>Delete an Item from a Cart<h3>";
   $sql = "select cartID, createdby from CART";
   $result = mysql_query($sql,$conn);
   if(mysql_num_rows($result) != 0)
   {
      echo "Carts: <select name=\"carts\">";
      while($val = mysql_fetch_row($result)) {
	 echo "<option value=$val[0]>$val[1], $val[0]</option>";
      }
      echo "</select>";
      $sql2 = "select name, productNum, barcode from ITEMS";
      $result2 = mysql_query($sql2,$conn);
      if(mysql_num_rows($result2) != 0) {
	 echo "Items: <select name=\"item\">";
